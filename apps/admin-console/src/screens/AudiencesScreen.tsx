      ...
      <div className="space-y-5">
        ...
        <form onSubmit={(e) => { e.preventDefault(); run(); }}>
          <Panel title="Kích hoạt audience" icon={<Send className="size-4" />}>
            ...
            <Field className="mt-3" label={`Danh sách OCC ID (mỗi dòng hoặc cách nhau dấu phẩy) — ${occIds.length} id`}>
              <Textarea aria-label="Danh sách OCC ID" value={raw} onChange={(e) => setRaw(e.target.value)} rows={5} className="font-mono text-xs" />
            </Field>
            <div className="mt-3">
