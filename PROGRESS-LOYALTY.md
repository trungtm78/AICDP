...
- [ ] **L1 — Kernel đa-currency public: convert/adjust/transfer** (RESUME TỪ ĐÂY): earn/reserve currency-aware public (optional currencyId, mặc định group); **convert** (đổi điểm brand→group / giữa currency qua point_conversion, double-entry cân per-currency, xử lý làm tròn floor + phần dư); **adjust** (±, lý do, audit, RBAC); **transfer** khách→khách; getBalance per-currency + listWallets (tất cả ví). API + schemas + FE cơ bản. TDD.
