...
- [ ] **Phase 5 — VN 1B + messaging 1B (RESUME TỪ ĐÂY)**: connector đặc thù VN. INBOUND: **KiotViet** (POS/retail webhook — nhận đơn), **GHN** (giao vận webhook cập nhật trạng thái đơn), **MISA** (kế toán/CRM — pull hoặc webhook). Map → ingest. OUTBOUND messaging: **VietGuys** (SMS brandname REST), **eSMS** (SMS/Zalo REST). Theo pattern có sẵn: inbound webhook (như Phase 2, token/brand từ connection) hoặc IPN-style (nếu có chữ ký); outbound (OutboundAdapter + safeFetch, như Zalo ZNS). Catalog đã có src cho một số kênh — thêm connector còn thiếu + adapter. TDD + e2e per task, checkpoint /review+/codex ranh giới phase. Sau đó Phase 6 (ERP REST/OData generic + preset SAP/Oracle) → 7 (Group2 stub honest + Frontend 2-group "đã/chưa setup" + hiện loại data + deploy).
