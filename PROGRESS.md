...
- [ ] **Phase 3 — Cổng thanh toán IPN (RESUME TỪ ĐÂY)**: adapter IPN cho VNPay/MoMo/ZaloPay. `@Public() POST /v1/connectors/payment/:id/ipn` xác thực CHECKSUM/chữ ký (dùng `signing.ts` hmacHex/verifyHmac + secret giải mã từ connection) theo spec từng cổng; map giao dịch thanh toán → `ingestOrderCompleted` (event_type payment) + `recordEvent`; trả response đúng định dạng cổng yêu cầu (vd VNPay RspCode). BẢO MẬT: verify checksum TRƯỚC (fail-closed), chống replay (idempotency theo txnRef), amount khớp, KHÔNG tin field client. Cần catalog thêm connector payment (src_vnpay/src_momo/src_zalopay) + config secret. TDD + e2e. Threat-model P0 "IPN-checksum" trong plan gốc.
