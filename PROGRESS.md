...
- [ ] **Phase 4 — OUTBOUND 1A + wiring activation (RESUME TỪ ĐÂY)**: adapter OUTBOUND thật (dùng `adapters/registry.registerOutbound` + `http-client.safeFetch` SSRF-safe) cho nhóm verify-free (analytics GA4/Amplitude, messaging webhook/Zalo ZNS...). Implement `OutboundAdapter{deliver,healthCheck}` cho vài đích; **wiring activation**: khi activation/journey chạy → gọi adapter deliver qua connection destination (gate consent ở activation ĐÃ có), ghi `connector_delivery` (recordDelivery đã có). `testConnection` (health.service) sẽ 'active' khi có adapter. FE nút test-send. TDD + e2e. Sau đó Phase 5 (VN KiotViet/GHN/MISA + Zalo ZNS/VietGuys/eSMS) → 6 (ERP REST/OData) → 7 (Group2 stub + Frontend 2-group + deploy).
